...
def setup_mid_dataset(output_dir):
    ...
    for split in ['train', 'val']:
        split_dir = os.path.join(RGBT_DATA_DIR, split)
        # Debug: hien thi cau truc thu muc neu can
        if os.path.isdir(split_dir):
            subdirs = [d for d in os.listdir(split_dir) if os.path.isdir(os.path.join(split_dir, d))]
            print(f'  [{split}] subdirs found: {subdirs}')
        else:
            raise FileNotFoundError(
                f'Split folder not found: {split_dir}\n'
                f'  -> Kiem tra data da upload chua: ls /root/AIP491/data/RGBTDronePerson/'
            )

        # Tim folder rgb: visible / rgb / images / RGB / Visible
        rgb_src = None
        for rgb_name in ['visible', 'rgb', 'images', 'RGB', 'Visible', 'color', 'Color']:
            p = os.path.join(RGBT_DATA_DIR, split, rgb_name)
            if os.path.isdir(p):
                rgb_src = p
                print(f'  [{split}] RGB folder: {rgb_name}/')
                break
        if rgb_src is None:
            raise FileNotFoundError(
                f'RGB folder not found in {split_dir}\n'
                f'  -> Subdirs co san: {subdirs}\n'
                f'  -> Dam bao upload_data.tar.gz da giai nen dung cau truc:\n'
                f'     {split_dir}/visible/  (hoac rgb/)'
            )

        # Tim folder thermal
        thr_src = None
        for thr_name in ['thermal', 'infrared', 'ir', 'Thermal', 'IR']:
            p = os.path.join(RGBT_DATA_DIR, split, thr_name)
            if os.path.isdir(p):
                thr_src = p
                print(f'  [{split}] Thermal folder: {thr_name}/')
                break
        if thr_src is None:
            thr_src = os.path.join(RGBT_DATA_DIR, split, 'thermal')  # fallback ke cu

        ann_src = None
        for ann_name in ['annotation', 'annotations', 'Annotations', 'labels', 'Labels']:
            p = os.path.join(RGBT_DATA_DIR, split, ann_name)
            if os.path.isdir(p):
                ann_src = p
                print(f'  [{split}] Annotation folder: {ann_name}/')
                break
        if ann_src is None:
            raise FileNotFoundError(
                f'Annotation folder not found in {split_dir}\n'
                f'  -> Subdirs co san: {subdirs}'
            )
...
def setup_late_dataset(output_dir):
    ...
    if os.path.isdir(check) and len(os.listdir(check)) > 0:
        ...
        for split in ['train', 'val']:
            ...
            rgb_src = None
            for rgb_name in ['visible', 'rgb', 'images', 'RGB', 'Visible', 'color']:
                p = os.path.join(RGBT_DATA_DIR, split, rgb_name)
                if os.path.isdir(p):
                    rgb_src = p
                    break
            ...
            ann_src = None
            for ann_name in ['annotation', 'annotations', 'Annotations', 'labels']:
                p = os.path.join(RGBT_DATA_DIR, split, ann_name)
                if os.path.isdir(p):
                    ann_src = p
                    break
