...
# Live mode config
DJI_API_KEY = os.environ.get("DJI_API_KEY", "")
DJI_LOG_EXE = os.environ.get("DJI_LOG_EXE", str(BASE_DIR / "dji-log.exe"))


def _find_adb_exe():
    """Tim adb.exe: trong Demo/, trong scrcpy bundle, hoac PATH."""
    candidates = [
        BASE_DIR / "adb.exe",
        *sorted(BASE_DIR.glob("scrcpy-*/adb.exe")),
    ]
    found = next((p for p in candidates if p.exists()), None)
    return str(found) if found else "adb"
...
# Capture status tracking (cho frontend biet khi capture loi)
capture_status: str = "idle"       # "idle" | "capturing" | "error" | "reconnecting"
capture_status_msg: str = ""       # Thong bao chi tiet cho frontend
...
def capture_worker():
    ...
    while True:
        ...
        while is_running and cap_rgb.isOpened():
            ...
            # Auto pseudo-thermal neu khong co thermal video
            if frame_thr is None:
...
async def upload_video(file: UploadFile = File(...),
    ...
    # Save Thermal video (optional — neu khong co thi auto-generate pseudo-thermal)
    thr_filename = ""
    if thermal_file is not None and thermal_file.filename:
        ...
        thr_filename = thermal_file.filename
        print(f"Thermal video saved: {thr_save}")
    else:
        thermal_video_path = None
        print("No thermal video — will auto-generate pseudo-thermal from RGB")
    ...
    return {"status": "ok", "filename": file.filename,
            "thermal_filename": thr_filename,
            "thermal_auto": thermal_video_path is None,
...
async def get_detections():
    ...
    if detector is not None:
        try:
            from src.Demo.inference import compute_gsd
    ...
    with state_lock:
        ...
        return JSONResponse(content={
            ...
            "capture_status_msg": capture_status_msg,
        })
...
def _find_windows(keyword="scrcpy"):
    ...
    # Giu reference cho callback — tranh GC khi goi tu background thread
...
def _is_window_valid(hwnd):
    """Check HWND con ton tai (window chua bi destroy)."""
    try:
        import ctypes
...
def live_capture_worker_hwnd(hwnd, width, height):
    ...
    Tu dong re-detect HWND khi scrcpy restart hoac capture loi lien tuc.
    """
    global live_mode, capture_status, capture_status_msg
    ...
    print(f"Live capture (HWND={current_hwnd}) started: {width}x{height}")

    while live_mode:
        t0 = time.time()

        # Check nhanh: HWND con ton tai khong? Neu khong thi skip capture, re-detect ngay
        if not _is_window_valid(current_hwnd):
            fail_count = max(fail_count, 29)  # Nhay thang len gan nguong re-detect

        frame_rgb = None
        if _is_window_valid(current_hwnd):
            frame_rgb = _capture_window_by_mss(current_hwnd)

        # Detect capture failure OR black screen (scrcpy mat ket noi)
        is_bad_frame = False
        if frame_rgb is None:
            is_bad_frame = True
        elif frame_rgb.mean() < 5.0:
            # Frame gan nhu toan den — scrcpy mat ket noi nhung window van ton tai
            is_bad_frame = True

        if is_bad_frame:
            fail_count += 1
            # Log khi bat dau loi (chi log 1 lan, khong spam)
            if fail_count == 30:
                capture_status = "error"
                if frame_rgb is not None:
                    capture_status_msg = "Scrcpy black screen — connection lost?"
                else:
                    capture_status_msg = "Cannot capture scrcpy window"
                print(f"[WARN] Capture failed {fail_count} consecutive frames (hwnd_valid={_is_window_valid(current_hwnd)})")
            # Moi ~3s thu tim lai HWND scrcpy (nhanh hon 10s cu)
            if fail_count >= 30 and fail_count % 30 == 0:
                capture_status = "reconnecting"
                capture_status_msg = "Re-detecting scrcpy window..."
                print(f"[INFO] Re-detecting scrcpy window... (current_hwnd={current_hwnd}, valid={_is_window_valid(current_hwnd)})")
                windows = _find_windows("scrcpy")
                print(f"[INFO] _find_windows('scrcpy') returned {len(windows)} windows: {[w['title'] for w in windows]}")
                if windows:
                    new_hwnd = windows[0]["hwnd"]
                    if new_hwnd != current_hwnd:
                        current_hwnd = new_hwnd
                        print(f"[INFO] Switching to new scrcpy HWND: {current_hwnd}")
                    # Reset va thu lai voi HWND moi (hoac cu)
                    fail_count = 0
                    capture_status = "capturing"
                    capture_status_msg = f"Re-connected: {windows[0]['title']}"
                    print(f"[INFO] Re-detected scrcpy: hwnd={current_hwnd}, title={windows[0]['title']}")
                else:
                    capture_status_msg = "Scrcpy not found, retrying..."
                    print("[WARN] Scrcpy window not found, will retry in ~3s")
            time.sleep(0.1)
            continue

        # Capture thanh cong
        if fail_count > 0:
            print(f"[INFO] Capture recovered after {fail_count} failed frames")
            capture_status_msg = ""
        fail_count = 0
        capture_status = "capturing"
...
def adb_preview_only_worker(device_serial: str):
    ...
    global adb_preview_mode
    adb_base = [ADB_EXE, "-s", device_serial] if device_serial else [ADB_EXE]
    ...
    try:
        while live_mode:
            ...
            try:
                ...
                if frame is None:
                    adb_fail_count += 1
                    if adb_fail_count == 50:  # ~2.5s
                        capture_status = "error"
                        capture_status_msg = "No frame from ADB stream"
                        print(f"[WARN] ADB capture: no frame for {adb_fail_count} attempts")
                    time.sleep(0.05)
                    continue
                if adb_fail_count > 0:
                    if adb_fail_count >= 50:
                        print(f"[INFO] ADB capture recovered after {adb_fail_count} empty frames")
                    adb_fail_count = 0
                    capture_status = "capturing"
                    capture_status_msg = ""
...
async def adb_devices():
    ...
    if not ok:
        return {"ok": False, "devices": [], "error": stderr}
    devices = []
    for line in stdout.splitlines()[1:]:  # Skip header "List of devices attached"
...
async def start_live(request: Request):
    ...
    # Auto-select device khi frontend khong gui adb_device
    if not adb_device:
        try:
            stdout, _, ok = _run_adb("devices")
            if ok:
                devs = [l.split()[0] for l in stdout.splitlines()[1:]
                        if l.strip() and len(l.split()) >= 2 and l.split()[1] == "device"]
                if len(devs) == 1:
                    adb_device = devs[0]
                    print(f"[INFO] Auto-selected ADB device: {adb_device}")
                elif len(devs) > 1:
                    # Nhieu device: chon device dau tien cho telemetry (tranh "more than one device")
                    adb_device = devs[0]
                    print(f"[INFO] Multiple devices ({len(devs)}), auto-selected first: {adb_device}")
        except Exception:
            pass
    ...
    try:
        from src.Demo.telemetry import TelemetryPoller
